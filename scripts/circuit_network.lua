...
perel.on_event(perel.events.on_built, function (event)
  ...
  if storage.wire_connection_target_cache[source_entity.name] and not invalid_wall(source_entity) then
    ...
    for _, wire_connector_id in pairs(nodes(source_entity.type)) do
      ...
      local existing_connections = perel.event_categories.circuit_network and 0 or 3
      local wire_connector = source_entity.get_wire_connector(wire_connector_id)
      if wire_connector then
        -- for each connection
        for _, wire_connection in pairs(wire_connector.real_connections) do
...
perel.on_event("perel-build", function (event)
  ...
  if storage.wire_connection_target_cache[destination_prototype.name] and not invalid_wall(wire_destination) then
    ...
    if wire_destination.unit_number ~= wire_source.unit_number and wire_destination.can_wires_reach(wire_source) then
      ...
      local event_names = {source_connector.is_connected_to(destination_connector) and "circuit_wire_removed" or "circuit_wire_added"}
